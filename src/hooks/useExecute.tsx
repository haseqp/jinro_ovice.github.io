...
export const useLastExecuted = () => {
  ...
  const lastDead = useMemo(
    () =>
      Array.from(gameRoles?.values()).filter((a) => a.deadAt === turn - 1)?.[0],
    [gameRoles, turn],
  );
  const lastDeadParticipant = useMemo(
    () =>
      lastDead === undefined ? undefined : findParticipantById(lastDead.id),
    [findParticipantById, lastDead],
  );
  ...
  useEffect(() => {
    const alives = (
      Array.from(gameRoles.values())
        .filter((role) => !(role.isDead ?? false))
        .map((role) =>
          participants.find(
            (participant: Participant) => participant.id === role.id,
          ),
        )
        .filter(
