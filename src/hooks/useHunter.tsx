...
export const useHunter = () => {
  ...
  const [protectedParticipants, setProtectedParticipants] = useAtom(
    protectedParticipantsAtom,
  );
  ...
  const protectableParticipants = useMemo(() => {
    ...
    gameRoles?.forEach((_value: GameRole, key: string) => {
      if (
        key !== myId &&
        !protectedParticipants.includes(key) &&
  ...
  const protect = useCallback(
    (id: string) => {
      if (
        protectableParticipants.find((participant) => participant.id === id) ===
        undefined
      ) {
        return;
      }
